...
var GraphComponent = (function (_super) {
    ...
    function GraphComponent(el, zone, cd, layoutService) {
        ...
        this.graphSubscription = new rxjs_1.Subscription();
        this.subscriptions = [];
        this.margin = [0, 0, 0, 0];
        this.results = [];
        ...
        this._oldLinks = [];
        this.transformationMatrix = transformation_matrix_1.identity();
    ...
    GraphComponent.prototype.draw = function () {
        ...
        var result$ = result instanceof rxjs_1.Observable ? result : rxjs_1.of(result);
        this.graphSubscription.add(result$.subscribe(function (graph) {
            _this.graph = graph;
            _this.tick();
        }));
        result$
            .pipe(operators_1.first(function (graph) { return graph.nodes.length > 0; }))
    ...
    GraphComponent.prototype.redrawLines = function (_animate) {
        ...
        this.linkElements.map(function (linkEl) {
            ...
            if (edge) {
                var linkSelection = d3_selection_1.select(linkEl.nativeElement).select('.line');
                linkSelection
                    .attr('d', edge.oldLine)
                    .transition()
                    .duration(_animate ? 500 : 0)
                    .attr('d', edge.line);
                var textPathSelection = d3_selection_1.select(_this.chartElement.nativeElement).select("#" + edge.id);
                textPathSelection
    ...
    GraphComponent.prototype.pan = function (x, y, zoomLevel) {
        ...
        this.transformationMatrix = transformation_matrix_1.transform(this.transformationMatrix, transformation_matrix_1.translate(x / zoomLevel, y / zoomLevel));
    ...
    GraphComponent.prototype.zoom = function (factor) {
        this.transformationMatrix = transformation_matrix_1.transform(this.transformationMatrix, transformation_matrix_1.scale(factor, factor));
        this.updateTransform();
    };
    /**
    ...
    GraphComponent.prototype.updateTransform = function () {
        this.transform = transformation_matrix_1.toSVG(this.transformationMatrix);
    };
    /**
     * Node was clicked
     *
    ...
    __decorate([
        core_1.ContentChild('clusterTemplate')
    ], GraphComponent.prototype, "clusterTemplate");
    __decorate([
        core_1.ContentChild('defsTemplate')
    ], GraphComponent.prototype, "defsTemplate");
    __decorate([
        core_1.ViewChild(ngx_charts_1.ChartComponent, { read: core_1.ElementRef })
    ], GraphComponent.prototype, "chart");
    __decorate([
        core_1.ViewChildren('nodeElement')
    ], GraphComponent.prototype, "nodeElements");
    __decorate([
        core_1.ViewChildren('linkElement')
    ], GraphComponent.prototype, "linkElements");
    __decorate([
        core_1.Input()
    ], GraphComponent.prototype, "groupResultsBy");
    __decorate([
        core_1.Input('zoomLevel')
    ], GraphComponent.prototype, "zoomLevel");
    __decorate([
        core_1.Input('panOffsetX')
    ], GraphComponent.prototype, "panOffsetX");
    __decorate([
        core_1.Input('panOffsetY')
    ], GraphComponent.prototype, "panOffsetY");
    __decorate([
    ...
    GraphComponent = __decorate([
        core_1.Component({
            ...
            animations: [animations_1.trigger('link', [animations_1.transition('* => *', [animations_1.animate(500, animations_1.style({ transform: '*' }))])])]
        })
    ], GraphComponent);
